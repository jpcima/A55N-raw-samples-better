...
alias sf_count_t = c_long;

enum SF_COUNT_MAX = 0x7FFFFFFFFFFFFFFF;
...
struct SF_CART_INFO
{
    ...
    char[64] client_id;
    char[64] category;
    char[64] classification;
    char[64] out_cue;
    char[10] start_date;
    char[8] start_time;
    char[10] end_date;
    char[8] end_time;
    char[64] producer_app_id;
    char[64] producer_app_version;
    char[64] user_def;
    int level_reference;
    SF_CART_TIMER[8] post_timers;
    char[276] reserved;
    char[1024] url;
    uint tag_text_size;
    char[256] tag_text;
...
alias sf_vio_get_filelen = c_long function (void* user_data);
alias sf_vio_seek = c_long function (sf_count_t offset, int whence, void* user_data);
alias sf_vio_read = c_long function (void* ptr, sf_count_t count, void* user_data);
alias sf_vio_write = c_long function (const(void)* ptr, sf_count_t count, void* user_data);
alias sf_vio_tell = c_long function (void* user_data);
...
{
    char[64] id; /* The chunk identifier. */
    uint id_size; /* The size of the chunk identifier. */
    uint datalen; /* The size of that data. */
...
** sf_close() is called.
*/
SF_CHUNK_ITERATOR* sf_get_chunk_iterator (
    SNDFILE* sndfile,
